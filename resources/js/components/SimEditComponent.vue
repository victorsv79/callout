<template>
    <div>
        <h3 class="text-center">Редактировать SIM-карту</h3>
        <div class="row">
            <div class="col-md-6">
                <form @submit.prevent="updateSim">
                    ...
                    <div class="form-group">
                        <label>Тариф</label>
                        <select class='form-control' v-model='sim.tarif_id'>
...
export default {
    ...
    created() {
        this.axios
            .get(`/api/sims/${this.$route.params.id}`)
            .then((res) => {
                this.sim = res.data;
            });
        this.axios
    ...
    methods: {
        updateSim() {
            this.axios
                .patch(`/api/sims/${this.$route.params.id}`, this.sim)
                .then((res) => {
                    this.$router.push({ name: 'sims' });
                })
                .catch(err => alert("Ошибка редактирования SIM, проверьте уникальность данных!"));
        }
    }
}
</script>
